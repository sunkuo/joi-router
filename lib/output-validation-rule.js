...
OutputValidationRule.prototype.validate = function (header, body) {
  ...
  if (this.schema.header) {
    debug('start output header validation')
    const headerValidateResult = Joi.validate(header, this.schema.header)
    if (headerValidateResult.error) {
      debug('output header valiation error')
      return headerValidateResult.error
    }
  }
  // validate body
  if (this.schema.body) {
    debug('start output body validation')
    const bodyValidateResult = Joi.validate(body, this.schema.body)
    if (bodyValidateResult.error) {
      debug('output body valiation error')
      return bodyValidateResult.error
    }
  }
}
