...
// {status, header, body}
const OutputValidationRule = function (status, schema) {
...
OutputValidationRule.prototype.validate = function (body) {
  ...
  if (this.schema) {
    ...
    const bodyValidateResult = Joi.validate(body, this.schema)
    if (bodyValidateResult.error) {
      debug('output body valiation rule failed')
      return bodyValidateResult.error
    }
    debug('output body validation rule success')
  }
}
